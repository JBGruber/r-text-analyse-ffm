...
# custom function
print_df <- function(x, max_r = 50L, max_c = 50) {
  if (is.dfm(x)) {
    x <- x %>%
      convert("data.frame")
  }
  ...
  x %>% 
    datatable(fillContainer = FALSE, rownames = FALSE, 
              options = list(pageLength = 5, scrollX = "200px", dom = "t")) %>%
...
predict(model, newdata = test_dfm)
```
...
- The process of standardizing, removing or merging features can have substantial important implications for the results:
  ...
  - negative: can lead to over-fitting or important features are removed by accident.
...
The result has changed from text3 being classified as 1 instead of 2.
...
The result is very similar except from the scale (i.e., distance between text) as texts seem to have become more similar.
