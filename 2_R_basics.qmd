---
title: "Session 2: R Basics Refresher"
...
The difference is that developers publish to CRAN less often (usually only when a significant improvement has been made to a package) and that some very new packages are not available on GitHub yet (others will never be available on CRAN for various reasons).
...
The example above is an experimental package I worked on a while ago and then did not have enough time to finish.
...
This can have some advantages, especially if you work with multiple packages that have functions of the same name (e.g., `plyr::count()` and `dplyr::count()`).
...
There are a couple of good sources for answers like <https://stackoverflow.com>, <https://community.rstudio.com/> different blogs and the R help list (<https://stat.ethz.ch/mailman/listinfo/r-help>).
...
This can easily be read back into `R` by anyone who wants to help you (don't worry if the code does not seem to make sense, it's purpose is to be read by `R`, not you):
...
Finally, ChatGPT knows `R` extremely well and if you phrase your question correctly, you can get high quality code exampes back.
You can also use it directly from `R` with my package [`askgpt`](https://github.com/JBGruber/askgpt).
...
Function are surprisingly easy to define in `R`.
You should define one once you notice that you run the same few lines of code again and again.
...
The difference is that `sapply` will try to simplify the output while `lapply` returns a list object by default:
...
That means `apply`-loops will destroy everything but one object created inside the function.
...
There is no way to determine how `sapply` returns your data though, which is a problem if your next step requires, for example, a character vector.
...
It evaluates a condition and only runs the code if the result is `TRUE`:
...
You can combine several `if` statements:
...
Some problems that drive many new `R` users nearly insane, arise from the factor class, for example.
...
An particular nuisance in this regard is that it's not always clear that a column in a `data.frame` is a factor:

```{r}
df <- data.frame(
  names = c("Jon", "David", "Theresa", "Rebecca", "Thomas", "Anna", "Jessica", "Saskia", "Mona", "Mark", "Lis", "Ben"),
  grades = c(15, 15, 14, 8, 5, 9, 10, 6, 6, 2, 0, 12),
  stringsAsFactors = TRUE # this was the default in older versions of R
)
df
df$names
```
...
```{r}
df_new <- df
df_new$names[7] <- "Jane" # replace the 7th name with Jane
df_new
```
...
Other operations simply won't work at all:
...
The `forcats` package makes working with factors a lot easier, but if you don't want to learn it right now, you should convert factors to strings, which makes them easier to work with:

```{r}
df_new$names <- as.character(df_new$names)
df_new$names[7] <- "Jane Doe"
df_new
```

## Hidden values in `R` objects

> "I sometimes find that there is “hidden” data that can appear if conjured but I don’t know what the magic spell is." -- student in my first R class
...
But to give you a quick heads up of the relevant points:
...
This object is a `data.frame`, which is a rectangular expression of data, meaning it has rows and columns like an Excel sheet.
...
Once you made a new class you can change the way `R` prints the object in the output and even the way it is displayed with `View`:
...
This is the reason why it sometimes seems like magic when you use commands like `print` or `plot` and `R` seems to know more about the object than meets the eye.
...
The `tibble` class just works the same way as `data.frame`s with only [a few small differences](https://cran.r-project.org/web/packages/tibble/vignettes/tibble.html).

The tidyverse functions are not quite as stable as *base* and the developers sometimes change or replace functions.
This means occasionally, something that worked before will give you a warning and if you ignore the warning for a year or two, you will get an error.
...
One famous example is the package `reshape2` that many people still use but which was superseded years ago by `tidyr`.
...
One of the big advances the `tidyverse` used to be that you could make code more readable by using pipes.
There is now a version of a pipe in the most recent versions of *base* `R`, but it is harder to use and still less common.
The most widely used pipe function comes from the `magrittr` package, which did not originate in the `tidyverse` but is today one of the most integral ones.
...
It solved one particular problem: readability.
...
The reason for this is that unless you understand a few basic principles, the grammar of graphics can seem like a completely foreign language (pun intended).
Again this is a quick walk through on what I consider to be these basic principles.
...
- **mapping = aes**: Next you define aesthetic mappings. When called inside `ggplot()`, the mappings are the default for the rest of the plot. That means unless you define different data to be on the y-axis, `ggplot2` will use the hp from here on.
...
The way these are structured are you use `scale_`, followed by the mapping you want a scale for (x, y and colour so far) and then then what scale you want.
...
Now in the plot it doesn't look so nice that we have the "as.factor(gear)" as the legend label.
...
And you can always define your own if you want a specific one for your own plotting needs.
  
# Important shortcuts in RStudio

The three keyboard shortcuts I use most are `alt` + `-` for `<-`, `Ctrl` + `alt` + `i` to insert a new code chunk in an R Markdown/Quarto document and `Ctrl` + `Shift` + `m` for inserting `%>% `.
